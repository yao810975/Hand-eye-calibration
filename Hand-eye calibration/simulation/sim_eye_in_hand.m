...
legend([h2 h4 h1 h3],'不进行标定件位置求解','不进行标定件位置求解只采用x方向数据','求解标定件位置','求解标定件位置只采用x方向数据',...
                                                                          'Box','off' );
...
x1 = fsolve(@(x) myfun_1(x,K,k,3*size(Pc)),[0;0;0;0;0],options);
...
% % 方法一求解完毕
%%  求解方式一只采用x方向数据
x3 = fsolve(@(x) myfun_1(x,K_x,k_x,size(Pc)),[0;0;0;0;0],options);
...
x2 = fsolve(@(x) myfun_2(x,kx,ky,kz,length(Pc)),[0;0],options);
...
%% 方法二只用x方向数据
x4 = fsolve(@(x) myfun_2(x,kx,kx,kx,length(Pc)),[0;0],options);
